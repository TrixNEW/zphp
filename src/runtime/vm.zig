    ...
    pub fn drainPendingDestruct(self: *VM) void {
            ...
            self.fiber_release_cursor < self.pending_fiber_release.items.len)
        {
            ...
            while (self.gen_release_cursor < self.pending_gen_release.items.len) {
                ...
                // rescued if re-retained between queue and drain. Multiple
                // zero-ref transitions may queue the same generator before a
                // drain; only the first entry may return it to the free list.
